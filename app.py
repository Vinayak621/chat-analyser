...
file_upload=st.sidebar.file_uploader("Select a file")
if file_upload is not None:
    ...
    if st.sidebar.button("Show analysis"):
        num_messages,words,msg_media,num_links=helper.fetch_stats(selected_user,df)
        ...
        if selected_user=='Overall':
            ...
            with col2:
                st.dataframe(new_df)
        df_wc=helper.create_wrd_cld(selected_user,df)
        fig,ax=plt.subplots()
        ax.imshow(df_wc)
        st.pyplot(fig)
        ...
        emoji_df=helper.emoji_used(selected_user,df)
